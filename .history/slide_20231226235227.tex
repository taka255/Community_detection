...
\begin{frame}
  \frametitle{Introduction: Why physics?}
  ...
  \end{itemize}
    ...
    \begin{columns}
      \begin{column}{0.4\textwidth}
        \begin{figure}
          \centering
          \includegraphics[width=0.6\linewidth]{figure/overfit.png}
      ...
      \begin{column}{0.60\textwidth}
        \begin{itemize}
          \item (Top) 38 edges crossing, (Bottom) 39 edges crossing
          \item The score is almost the same, but the community is completely different.
          $\Rightarrow$ Finding ground state is not enough!!
        \end{itemize}
      \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    \item The rigorous information-theoretical and computational threshold in assortative SBM (i.e. $c_{in} > c_{out}$).
    \item The rigorous proof that reconstruction is hard for specific algorithms in the hard regimes.\\
    (e.g. Do MCMC or BP with random initial conditions really take exponential time?)
    \item In hard regimes, Is it possible to solve efficiently by providing some correct labels (side information) ?
  \end{itemize}
