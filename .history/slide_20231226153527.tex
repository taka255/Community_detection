...
\begin{frame}
  ...
  \begin{itemize}
    \item Belief propagation (BP): method in statistical physics for computing marginal distributions $p(\sigma_i \mid G)$ of a graphical model.
    \item For simplicity, we consider the case where $G$ is a tree.
    \begin{itemize}
      \item \alert{message}: $m_{i \to j}(\sigma_j) = p(\sigma_j \mid G_{ij})$ where $G_{ij}$ is the subgraph of $G$ induced by the nodes in the path between $i$ and $j$.
    \end{itemize}
  \end{itemize}
