...
\begin{frame}
  ...
  \end{itemize}
  Proof: Suppose there is an algorithm for weak reconstruction, which means that we can say ``there is a community'' to $G$ in SBM with high probability. \\
  Then by the same algorithm, we can say ``there is community'' to $G$ in Eronds-Renyi model with high probability, which is incollect \\
